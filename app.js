/* Room Expense Manager - shared Supabase backend
   Real application data is stored in Supabase. localStorage is NOT used as a database.
*/
...
function calc(k = currentKey()) {
  ...
  for (const c of creditors) {
    for (const d of debtors) {
      if (c.amount > 0.005 && d.amount > 0.005) {
        const a = Math.min(c.amount, d.amount);
        settlements.push({ from: d.name, to: c.name, amount: a });
        c.amount -= a;
        d.amount -= a;
      }
    }
